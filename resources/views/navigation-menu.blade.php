<nav x-data="{ open: false }" class="bg-zinc-800 dark:bg-zinc-900 border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden xl:flex xl:items-center xl:ml-6">
                ...
                <div class="flex gap-5 h-full items-center">
                    ...
                    <div class="text-zinc-200 h-full flex justify-center items-center">|</div>
                    <div class="ml-3 relative">
                                ...
                                @else
                                    <span class="inline-flex rounded-md">
                                        <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-zinc-800 dark:bg-zinc-900 hover:bg-zinc-700 dark:hover:bg-zinc-800 focus:bg-zinc-900 focus:outline-none transition">
                                            ...
                                            @if(Auth::user()->job=='A')
                            Administrador
                            @elseif(Auth::user()->job=='T')
                            Trabajador
                            @elseif(Auth::user()->job=='M')
                            Mecanico
                            @endif
            ...
            <div class="flex justify-center items-center gap-2 relative w-8">
                            <button class="bg-zinc-800 dark:bg-zinc-900 hover:bg-zinc-700 dark:hover:bg-zinc-800 duration-200 shadow-2xl w-8 h-8 flex justify-center items-center rounded-lg absolute" id="sun2">
                                <div class="h-full w-full flex justify-center items-center sundiv2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-yellow-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                            </svg>
                                </div>
                            </button>
                            <button class="bg-zinc-800 dark:bg-zinc-900 hover:bg-zinc-700 dark:hover:bg-zinc-800 duration-200 shadow-2xl w-8 h-8 flex justify-center items-center rounded-lg text-white absolute dark:invisible dark:opacity-0" id="moon2">
                            <div class="h-full w-full flex justify-center items-center moondiv2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                            </svg>
                            </div>
            ...
            <div class="-mr-2 flex items-center">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-zinc-900 focus:outline-none focus:bg-zinc-700 focus:text-gray-100 transition">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden xl:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-jet-responsive-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')" class="text-white">
                {{ __('Dashboard') }}
            </x-jet-responsive-nav-link>
                    
            @if(Auth::user()->job!='M')
                    <x-jet-responsive-nav-link href="{{ route('users.index') }}" :active="request()->routeIs('users.index')" class="text-white">
                        {{ __('Usuarios') }}
                    </x-jet-responsive-nav-link>
                    @endif
                    <x-jet-responsive-nav-link href="{{ route('customers.index') }}" :active="request()->routeIs('customers.index')" class="text-white">
                        {{ __('Clientes') }}
                    </x-jet-responsive-nav-link>
                    <x-jet-responsive-nav-link href="{{ route('vehicles.index') }}" :active="request()->routeIs('vehicles.index')" class="text-white">
                        {{ __('Vehiculos') }}
                    </x-jet-responsive-nav-link>
            @if(Auth::user()->job!='M')
                    <x-jet-responsive-nav-link href="{{ route('providers.index') }}" :active="request()->routeIs('providers.index')" class="text-white">
                        {{ __('Proveedores') }}
                    </x-jet-responsive-nav-link>
                    @endif
                    <x-jet-responsive-nav-link href="{{ route('products.index') }}" :active="request()->routeIs('products.index')" class="text-white">
                        {{ __('Inventario') }}
                    </x-jet-responsive-nav-link>
                    <x-jet-responsive-nav-link href="{{ route('reporting.index') }}" :active="request()->routeIs('reporting.index')" class="text-white">
                        {{ __('Reporte') }}
                    </x-jet-responsive-nav-link>
            @if(Auth::user()->job!='M')
                    <x-jet-responsive-nav-link href="{{ route('billing.index') }}" :active="request()->routeIs('billing.index')" class="text-white">
                        {{ __('Facturación') }}
                    </x-jet-responsive-nav-link>
                    @endif
        </div>
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}" x-data>
                    ...
                    <x-jet-responsive-nav-link href="{{ route('logout') }}"
                                   @click.prevent="$root.submit();">
                        <p class="text-white">{{ __('Cerrar sesión') }}</p>
                    </x-jet-responsive-nav-link>
                </form>
